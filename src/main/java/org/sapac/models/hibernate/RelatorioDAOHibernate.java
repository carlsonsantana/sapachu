...
public class RelatorioDAOHibernate extends GenericDAOHibernate implements RelatorioDAO {
	...
	public Collection<Map<String, String>> gerarRelatorioDiferenca(MembroEquipe membroEquipe) {
		...
		hql.append("SELECT SUM(COALESCE(situacao_anterior.area, 0) - COALESCE(situacao_posterior.area, 0)) AS diferenca_area, ")
			...
			.append("	consulta.id_consulta, consulta.data_consulta FROM ( ")
			...
			.append("		INNER JOIN sapac.membro_participa_consulta AS membros_consulta ON (membros_consulta.id_consulta = consulta_anterior.id_consulta) ")
			.append("		INNER JOIN ( ")
			...
			.append("	GROUP BY consulta.id_consulta, consulta.data_consulta, variaveis_clinicas_anterior.pressao_arterial - variaveis_clinicas_posterior.pressao_arterial, ")
